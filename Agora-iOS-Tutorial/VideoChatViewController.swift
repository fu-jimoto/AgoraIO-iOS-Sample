...
class VideoChatViewController: UIViewController {
    @IBOutlet weak var resolution: UISlider!
    ...
    override func viewDidLoad() {
        ...
        setupLocalVideo()
        joinChannel()
        resolution.addTarget(self, action: #selector(sliderDidChangeValue(_:)), for: .valueChanged)
    }

    @objc func sliderDidChangeValue(_ sender: UISlider) {
        let value = sender.value
        sender.value = round(value / 0.5) * 0.5
        
        switch String(value) {
        case "0.0":
            agoraKit.setVideoEncoderConfiguration(AgoraVideoEncoderConfiguration(size: AgoraVideoDimension160x120,
                                                                                 frameRate: .fps15,
...
extension VideoChatViewController: AgoraRtcEngineDelegate {
    ...
    func rtcEngine(_ engine: AgoraRtcEngineKit, networkQuality uid:UInt, txQuality:AgoraNetworkQuality, rxQuality:AgoraNetworkQuality){
        if(uid == 0){
            logVC?.log(type: .info, content: "NWQuality: \(txQuality.rawValue)")
        }
    }
}
